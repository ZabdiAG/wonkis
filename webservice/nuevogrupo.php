...
if (!empty($_POST)) {
    ...
    if (empty($_POST['nom_grupo'])) {
        
        
        // Create some data that will be the JSON response 
        $response["success"] = 0;
        $response["message"] = "Ingresa el nombre del grupo porfavor!";
    ...
    $query        = " SELECT 1 FROM grupo WHERE nombre = :nom_grupo";
    ...
    if ($row) {
        ...
        $response["success"] = 0;
        $response["message"] = "Este nombre de grupo ya esta en uso";
        die(json_encode($response));
    }
    ...
    $query = "INSERT INTO grupo ( nombre ) VALUES ( :nom_grupo ) ";
